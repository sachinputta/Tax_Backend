...
public class AdminImpl implements AdminService {
	...
	@Override
	public Customer updateAdmin(String customerId, Customer updatedCustomer) throws InvalidAdminId {
		...
		if (updatedCustomer.getCustomerPassword() != null)
			customer.setCustomerPassword(encoder.encode(updatedCustomer.getCustomerPassword()));
	...
	public List<Customer> getCustomersByRole(String roleName) {
		List<Customer> filteredCustomers = customerRepository.findAll().stream()
				.filter(c -> c.getRoles().stream().anyMatch(role -> role.getRoleName().equalsIgnoreCase(roleName)))
				.sorted(Comparator.comparing(Customer::getCompanyName)).collect(Collectors.toList());
		return filteredCustomers;
	}

	public int getCustomersNoByRole(String roleName) {
		return (int) customerRepository.findAll().stream()
				.filter(c -> c.getRoles().stream().anyMatch(role -> role.getRoleName().equalsIgnoreCase(roleName)))
				.count();
	}

	@Override
	public byte[] getProfilePicture(String customerId) throws IOException {
		Customer customer = customerRepository.findByCustomerId(customerId);
		if (customer == null || customer.getImagePath() == null || customer.getImagePath().isEmpty()) {
			throw new IllegalArgumentException("Customer does not have a photo.");
		}
		return Files.readAllBytes(Paths.get(customer.getImagePath()));
	}

	@Override
	@Transactional
	public void updatePhoto(String customerId, MultipartFile photo) throws Exception {
		Customer customer = customerRepository.findById(customerId)
				.orElseThrow(() -> new Exception("Customer not found"));

		deletePhotoFromFileSystem(customer.getImagePath());
		String imagePath = savePhotoToFileSystem(photo);
	...
	@Override
	@Transactional
	public void uploadPhoto(String customerId, MultipartFile file) throws Exception {
		String uniqueFileName = UUID.randomUUID().toString() + "_" + file.getOriginalFilename();
		...
		customer.setImagePath(filePath);
		customerRepository.save(customer);
	}

	@Override
	public List<Long> getAllPhoneNumbers() {
		return customerRepository.findAllPhoneNumbers();
	}

	@Override
	public String sendRequestReply(int ticketId, String customerId, String replyMsg)
			throws InvalidIdException, UnsupportedEncodingException, MessagingException {
		return null;
	}
	...
	@Override
	public List<Customer> getCustomersNotAdmin() {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	public int getCustomersCountByRole(String roleName) {
		// TODO Auto-generated method stub
		return 0;
	}
	...
	@Override
	public boolean checkEmailToUpdate(String customerId, String email) {
		// TODO Auto-generated method stub
		return false;
	}

	@Override
	public boolean checkPhoneNumberToUpdate(String customerId, long phoneNumber) {
		// TODO Auto-generated method stub
		return false;
	}
}
